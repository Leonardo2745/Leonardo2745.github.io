...
var _resultViewJs = require("./view/result-view.js");
let data = [];
const submitType = {
    NEW: 0,
    UPDATE: 1
...
const clickEsquerdo = (event)=>{
    ...
    alert(`Clicou com o bot\xe3o esquerdo, e o ${data[currentId].getNome().toUpperCase()} ser\xe1 carregado para edi\xe7\xe3o`);
    const confirmUpdate = window.confirm("Voc\xea realmente deseja atualizar este usu\xe1rio?");
    if (confirmUpdate) {
...
const clickDireito = (event)=>{
    ...
    if (event.button == 2) {
        currentId = event.target.closest("tr").id.split("")[4];
        alert(`Clicou com o bot\xe3o direito, e o ${data[currentId].getNome().toUpperCase()} ser\xe1 deletado`);
        const confirmDelet = window.confirm("Voc\xea realmente deseja deletar este usu\xe1rio?");
        if (confirmDelet) {
            deletUser(currentId);
            (0, _resultViewJs.resultView).update(data);
        }
    }
};
const controller = {
